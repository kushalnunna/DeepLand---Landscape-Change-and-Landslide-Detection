...
with center_column:
    if st.button('Submit'):
        ...
        result = subprocess.run(['python', r'C:\Users\DELL\Desktop\FYP\Landscape-Change-Detection-main\predict.py',
                                 '--input', 'temp_images', '--output', 'temp_images','-t','0.1' , '-c' ,'black'],
                                 capture_output=True, text=True)
        st.write("Subprocess output (predict.py):", result.stdout)
        st.write("Subprocess error (predict.py, if any):", result.stderr)

        st.write('Model execution complete. Running mask generation script...')

        # Run the mask.py script
        result = subprocess.run(['python', r'C:\Users\DELL\Desktop\FYP\Landscape-Change-Detection-main\mask.py'],
                                 capture_output=True, text=True)
        st.write("Subprocess output (mask.py):", result.stdout)
        st.write("Subprocess error (mask.py, if any):", result.stderr)

        # Check the files in the directory before trying to load the result
        st.write("Files in temp directory:", os.listdir(os.path.join(temp_dir, temp_dir2)))

        # Load and display the predicted images
        with col2:
        ...
        with col3:
            ...
            if os.path.exists(result_image_path):
                ...
                # Save the result image to the output_images directory
                shutil.copy(result_image_path, os.path.join(output_dir, f'result_instance_{instance_num}.png'))
                st.write(f'Result image saved to {output_dir}/result_instance_{instance_num}.png')
            else:
